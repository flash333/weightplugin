...
package org.opendaylight.weightplugin.protocol;
 ...
 */
public class WeightPluginPhysicalPort implements Cloneable, Serializable {
    public static int MINIMUM_LENGTH = 48;
    public static int WEIGHT_ETH_ALEN = 6;

    public WeightPluginPhysicalPort(short portNumber){
        this.portNumber = portNumber;
    }
    public WeightPluginPhysicalPort(){
    }

    public enum WeightPluginPortConfig {
    	WeightPluginPPC_PORT_DOWN    (1 << 0),
    	WeightPluginPPC_NO_STP       (1 << 1),
    	WeightPluginPPC_NO_RECV      (1 << 2),
    	WeightPluginPPC_NO_RECV_STP  (1 << 3),
    	WeightPluginPPC_NO_FLOOD     (1 << 4),
    	WeightPluginPPC_NO_FWD       (1 << 5),
    	WeightPluginPPC_NO_PACKET_IN (1 << 6);

        protected int value;

        private WeightPluginPortConfig(int value) {
            this.value = value;
        }
    ...
    public enum WeightPluginPortState {
    	WeightPluginPPS_LINK_DOWN   (1 << 0),
    	WeightPluginPPS_STP_LISTEN  (0 << 8),
        WeightPluginPPS_STP_LEARN   (1 << 8),
        WeightPluginPPS_STP_FORWARD (2 << 8),
        WeightPluginPPS_STP_BLOCK   (3 << 8),
        WeightPluginPPS_STP_MASK    (3 << 8);
    	WeightPluginPPS_STP_CHANGE  (3 << 8);

        protected int value;

        private WeightPluginPortState(int value) {
            this.value = value;
        }
    ...
    public void setHardwareAddress(byte[] hardwareAddress) {
        if (hardwareAddress.length != WeightPluginP_ETH_ALEN)
            throw new RuntimeException("Hardware address must have length "
                    + WeightPluginP_ETH_ALEN);
    ...
    public void readFrom(ByteBuffer data) {
        ...
        if (this.hardwareAddress == null)
            this.hardwareAddress = new byte[WeightPluginP_ETH_ALEN];
    ...
    public boolean equals(Object obj) {
        ...
        if (!(obj instanceof WeightPluginPhysicalPort)) {
            return false;
        }
        WeightPluginPhysicalPort other = (WeightPluginPhysicalPort) obj;
    ...
    public WeightPluginPhysicalPort cloneWeightPluginPhysicalPort() {
        WeightPluginPhysicalPort p;
        try
        {
            p = (WeightPluginPhysicalPort) this.clone();
