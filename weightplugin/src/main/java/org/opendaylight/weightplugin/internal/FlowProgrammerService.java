...
package org.opendaylight.weightplugin.internal;//s4s
...
import org.opendaylight.weightplugin.IFlowProgrammerNotifier;//s4s
import org.opendaylight.weightplugin.IInventoryShimExternalListener;//s4s
import org.opendaylight.weightplugin.core.IController;//s4s
//import org.opendaylight.controller.protocol_plugin.openflow.core.IMessageListener;//s4s //receive()
import org.opendaylight.weightplugin.core.ISwitch;//s4s
...
/**
 * Represents the weightplugin plugin component in charge of programming the flows
...
//s4s mark 2
    private Status modifyFlowInternal(Node node, Flow oldFlow, Flow newFlow, long rid) {
        ...
        if (controller != null) {
            ISwitch sw = controller.getSwitch((Long) node.getID());
            if (sw != null) {
                /* //OF's code require Flow converted to OFMessage; similarly, weightplugin use SNMPMessage
